...
void operate_on_columns(double** matrix, int m, int column, double multiply_by, int sum_to) {
	...
	if(sum_to == -1) { // Operations should stay in the same column
		for(i = 0; i < m; i++) {
			...
			if(fabs(matrix[i][column]) < 0.00001) {
				matrix[i][column] = 0;
			}
		}	
	}
...
// For columns where c is negative multiply the entire column by -1
...
int find_non_zero_element(double** matrix, int m, int column) {
	...
	for(i = 1; i < m; i++) {
		// Probably this needs to be > 0, otherwise it can screw up the simplex
		if(matrix[i][column] != 0) {
			return i;
		}
	}
...
void format_canonical(double** matrix, int m, int n, int* base) {
	...
	for(i = 0; i < (m - 1); i++) { // Goes through all the basic columns
		if(matrix[i + 1][base[i]] == 0) { // If the row for the base is 0, find other row on the same column that can make the first one != 0
